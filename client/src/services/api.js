...
const api = axios.create({
  baseURL: "https://arif-library-system.onrender.com/api",
  headers: {
...
export const adminAPI = {
  ...
  updateBookCopies: (bookId, copiesAvailable) =>
    api.put(`/admin/update-book-copies/${bookId}`, { copiesAvailable }),
  ...
  approveRequest: (requestId) =>
    api.put(`/admin/requests/${requestId}/approve`),
  rejectRequest: (requestId, reason) =>
    api.put(`/admin/requests/${requestId}/reject`, { reason }),
...
export const issueAPI = {
  ...
  rejectRequest: (issueId, rejectionReason) =>
    api.post("/issues/reject", { issueId, rejectionReason }),
  issueBookDirectly: (issueId) =>
    api.post("/issues/issue-directly", { issueId }),
  returnBookDirectly: (issueId) =>
    api.post("/issues/return-directly", { issueId }),
...
export const questionPaperAPI = {
  ...
  uploadQuestionPaper: (formData) =>
    api.post("/question-papers", formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }),
  downloadQuestionPaper: (id) =>
    api.get(`/question-papers/${id}/download`, {
      responseType: "blob",
    }),
...
export const questionPaperRequestAPI = {
  submitRequest: (formData) =>
    api.post("/question-paper-requests", formData, {
      headers: {
        "Content-Type": "multipart/form-data",
      },
    }),
  ...
  deleteRequest: (id) =>
    api.delete(`/question-paper-requests/my-requests/${id}`),
  getAllRequests: (params) => api.get("/question-paper-requests", { params }),
  getRequestStats: () => api.get("/question-paper-requests/stats"),
  approveRequest: (id, feedback) =>
    api.put(`/question-paper-requests/${id}/approve`, { feedback }),
  rejectRequest: (id, feedback) =>
    api.put(`/question-paper-requests/${id}/reject`, { feedback }),
